...
!(function($) {
	...
	function onClose(e) {
		...
		var isSelf = $item.hasClass('modal');
		...
		var $modal = isSelf ? $item : $item.closest('.modal');
		close($modal);
	}
...
!(function($) {
	...
	function init($modal) {
		...
		$modal.on('close.modal', onHide);
		$modal.on('open.modal', onShow);
	}
...
/// ----------------------------
/// Обработка сетки изображений.
/// ----------------------------
!(function($) {

  function init($grid) {
    var $childs = $grid.children();
    var first = $childs.first().get(0);
...
!(function($) {
	...
	function getEnters() {
		return $_enters ? $_enters : ($_enters = $('[data-scroll-enter]'));
	}
...
!(function($) {
  ...
  function onChange(e, index) {
    ...
    var func = index === 0 ? 'removeClass' : 'addClass';
    $lead[func]('plain');
  }
